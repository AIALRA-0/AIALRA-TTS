...
4. Review the production config in `deploy/config.remote.yaml`. It is rendered from `deploy/config.remote.example.yaml` with the generated local secrets, so `deploy-check` can read it directly.
   ...
   - Keep `webui.csrf_origin_check=true` on Contabo so browser cookie-authenticated writes must come from the public WebUI origin. Add only deliberate same-site reverse-proxy origins to `webui.csrf_trusted_origins`.
...
11. Validate:
   ...
   - archived project and folder restore actions make targets selectable for new jobs again without changing historical job metadata
